CREATE   VIEW dbo.zvwBatchDeleteCodeGen
AS

SELECT 'EXECUTE @RC = dbo.stpHouseRulesAgreementDelete ' + CHAR(39) + EMail + CHAR(39) AS CodeGen
FROM dbo.HouseRulesAgreement
...
Step 2: execute the view below UNION script to create a CodeGen of records to Delete
  ...
  UNION
	SELECT CodeGen FROM dbo.zvwBatchDeleteCodeGen
...
SELECT COUNT(*) AS RowCnt, 'Donation' AS TableName							FROM dbo.Donation
UNION
SELECT COUNT(*) AS RowCnt, 'Registration' AS TableName					FROM dbo.Registration
UNION
SELECT COUNT(*) AS RowCnt, 'HouseRulesAgreement' AS TableName	FROM dbo.HouseRulesAgreement
