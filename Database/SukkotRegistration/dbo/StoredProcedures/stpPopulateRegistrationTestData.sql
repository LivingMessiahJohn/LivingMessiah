...
CREATE PROCEDURE dbo.stpPopulateRegistrationTestData (
@EMail as nvarchar(75),
@Scenario as nvarchar(75)
)
AS
/*
	DECLARE @RC int; EXEC @RC = dbo.stpPopulateRegistrationTestData 'aeaij@yahoo.com', 'AgreementNotSigned'
	DECLARE @RC int; EXEC @RC = dbo.stpPopulateRegistrationTestData 'aeaij@yahoo.com', 'StartRegistration'
	DECLARE @RC int; EXEC @RC = dbo.stpPopulateRegistrationTestData 'aeaij@yahoo.com', 'Payment-Partial'
	DECLARE @RC int; EXEC @RC = dbo.stpPopulateRegistrationTestData 'aeaij@yahoo.com', 'Payment-Full'
	DECLARE @RC int; EXEC @RC = dbo.stpPopulateRegistrationTestData 'aeaij@yahoo.com', 'Complete'
	DECLARE @RC int; EXEC @RC = dbo.stpPopulateRegistrationTestData 'aeaij@yahoo.com', 'Test Unrecognized Scenario'
...
BEGIN TRY
	...
	EXECUTE @RC = dbo.stpHouseRulesAgreementDelete @Email
  IF @Scenario = 'AgreementNotSigned' 
	...
	ELSE IF @Scenario = 'StartRegistration'
		BEGIN
			INSERT INTO dbo.HouseRulesAgreement (
	...
	ELSE IF @Scenario = 'Payment-Partial'
		BEGIN
			...
			SELECT @StatusId = StatusPaymentId FROM dbo.Constants;
			SET @FirstName='Michael'
			SET @FamilyName = 'Palin'

			-- @HouseRulesAgreementId, 
			EXECUTE @RC = dbo.stpRegistrationInsert 
			...
			EXEC @RC = dbo.stpDonationInsert @NewRegistrationId, 30.00, 'test, add half of registraion', 'Confirmation# XXXXXX', 'Admin@LivingMessiah.com', @dt, @NewDonationId OUTPUT
	...
	ELSE IF @Scenario = 'Payment-Full'
		BEGIN
			...
			EXEC @RC = dbo.stpDonationInsert @NewRegistrationId, 60.00, 'test, add all of registraion', 'Confirmation# XXXXXX', 'Admin@LivingMessiah.com', @dt, @NewDonationId OUTPUT
			SELECT @RC AS RC, @NewDonationId AS NewId
	...
	ELSE IF @Scenario = 'Complete'
		BEGIN
			...
			SELECT @StatusId = StatusCompleteId  FROM dbo.Constants;
			SET @FirstName='John'
			...
			EXEC @RC = dbo.stpDonationInsert @NewRegistrationId, 60.00, 'test, full registraion', 'Confirmation# XXXXXX', 'Admin@LivingMessiah.com', @dt, @NewDonationId OUTPUT
			SELECT @RC AS RC, @NewDonationId AS NewId
