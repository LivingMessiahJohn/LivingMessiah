CREATE PROCEDURE dbo.stpDonationInsert
(
	@RegistrationId int,
	@Amount MONEY,
	@Notes nvarchar(255),
...
SELECT * FROM zvwErrorLog WHERE ErrorProcedure = 'dbo.stpDonationInsert'
...
SELECT  @StatusIdComplete = StatusCompleteId FROM dbo.Constants;
...
BEGIN TRY
	...
	SELECT
	 @RegistrationId
	 , ISNULL((SELECT MAX(Detail) FROM dbo.Donation WHERE @RegistrationId = dbo.Donation.RegistrationId), 0) + 1 AS Detail
	...
	*/
	...
	UPDATE dbo.Registration SET StatusId = @StatusIdComplete WHERE Id = @RegistrationId

	PRINT 'Delete from dbo.Stripe '
	DELETE dbo.Stripe WHERE Email=@Email
