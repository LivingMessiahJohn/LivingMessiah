...
CREATE    PROCEDURE dbo.stpStripeMerge
(
    @Email nvarchar(75)
    , @RegistrationId int
    , @NewId int OUTPUT
...
--DELETE dbo.Stripe WHERE Id=1

SELECT * FROM zvwErrorLog WHERE ErrorProcedure = 'dbo.stpStripeMerge'

GRANT EXECUTE ON dbo.stpStripeMerge TO _________  
...
BEGIN TRY
    ...
    MERGE INTO dbo.Stripe AS target
    USING (
        SELECT 
            @Email AS Email,
