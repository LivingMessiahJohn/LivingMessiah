...
public abstract class Triennial : SmartEnum<Enums.Triennial>
{
  ...
  public string AllAbrv
  {
    get
    {
      var parts = new List<string>
      {
        $"{BibleBook.FromValue(this.TorahVerse.BibleBook).Abrv} {this.TorahVerse.ChapterVerse}"
      };
      /*  */
      if (HaftorahVerses is not null)
      {
        parts.Add($"Haftorah: {HaftorahAbrv}");
      }

      if (BritVerses is not null)
      {
        parts.Add($"Brit: {BritAbrv}");
      }
      return String.Join("; ", parts);
    }
  }

  //public string SpanIconAndTextAll => $"<span class='badge bg-danger-subtle text-dark'><i class='far fa-bookmark'></i> {AllAbrv}</span>";
  //public string SpanIconAndTextTorah => $"<span class='badge bg-danger-subtle text-dark'><i class='far fa-bookmark'></i> {TorahAbrv}</span>";
  public string SpanIconAndTextTorah => $"<span class='badge bg-danger-subtle text-dark'>{TorahAbrv}</span>";
