...
public abstract class EventMoon : SmartEnum<EventMoon>
{
  ...
  //public abstract string JustifyContentSuffix { get; }  // [ end | start | center]
  ...
  public DateOnly? DayTimeDate => Date;  // DateType.Month.Icon
  //public string JustifyContent => $"justify-content-{JustifyContentSuffix}";
  public string SpanIcon => $"<span class='badge {DateType.Month.BadgeColor}'><i class='{DateType.Month.Icon} '></i></span>";
  public string SpanText => $"<span class='badge {DateType.Month.BadgeColor} {DateType.Month.TextColor}'> {Name}</span>";

  //.Select(m => new Enums.EventRecord(m.Date ?? default, DateType.Month, m.Value, m.FullName,  ConstantsCSSTextAlign.Start)))
  //public DateOnly ErevDateWithDefault => Date.HasValue ? Date.Value.AddDays(-1) : default;
  //public DateOnly DayTimeDateWithDefault => Date ?? default;
